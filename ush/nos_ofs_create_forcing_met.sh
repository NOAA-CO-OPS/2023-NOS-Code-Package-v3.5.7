...
#     ${PREFIXNOS}.hflux.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     Fortran.${RUN}.nowcast.log
#     ----  forecast ----
#     ${PREFIXNOS}.met.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.hflux.forecast.$yyyy$mm$dd.t${cyc}z.nc 
  ...
  if [ $exit_status -eq 0 ]; then
    ...
    if [ $OCEAN_MODEL == 'SELFE' -o $OCEAN_MODEL == 'selfe' ]; then
      if [ -s sflux_air.nc ];   then 
       ...
       cp -p sflux_air.nc ${PREFIXNOS}.air.$RUNTYPE.$PDY1.t${cyc}z.nc
      fi
      if [ -s sflux_rad.nc ];   then
       cp -p sflux_rad.nc sflux_rad_1.001.nc 
       cp -p sflux_rad.nc ${PREFIXNOS}.flux.$RUNTYPE.$PDY1.t${cyc}z.nc
      fi
      if [ -s sflux_prc.nc ];   then
       cp -p sflux_prc.nc sflux_prc_1.001.nc
       cp -p sflux_prc.nc ${PREFIXNOS}.precip.$RUNTYPE.$PDY1.t${cyc}z.nc
      fi
