...
# Nowcast  
# Input:
#     ${PREFIXNOS}.river.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.obc.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.met.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.init.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.hflux.nowcast.$yyyy$mm$dd.t${HH}z.nc
#     ${RUN}_roms_nowcast.in
#     ${PREFIXNOS}.roms.tides.nc
# Output:
#     ${PREFIXNOS}.stations.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.fields.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.fields.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.rst.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.roms.nowcast.$yyyy$mm$dd.t${cyc}z.log
# Forecast  
# Input:
#     ${PREFIXNOS}.river.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.obc.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.met.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.rst.nowcast.$yyyy$mm$dd.t${cyc}z.nc
#     ${RUN}_roms_forecast.in
#     ${PREFIXNOS}.roms.tides.nc
# Output:
#     ${PREFIXNOS}.stations.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.fields.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.rst.forecast.$yyyy$mm$dd.t${cyc}z.nc
#     ${PREFIXNOS}.roms.forecast.$yyyy$mm$dd.t${cyc}z.log
#
...
#    NQCK_INTERVAL=`expr $NQCK_INTERVAL / 3600`
    ...
    if [ -f ${PREFIXNOS}.avg.nc ]; then 
      mv ${PREFIXNOS}.avg.nc ${PREFIXNOS}.avg.nowcast.$PDY.t${cyc}z.nc
      cp -p ${PREFIXNOS}.avg.nowcast.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f damee4_fwd_outer0_0001.nc ]; then
      mv damee4_fwd_outer0_0001.nc ${PREFIXNOS}.fwd_outer0.$PDY.t${cyc}z.nc
      cp -p ${PREFIXNOS}.fwd_outer0.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f damee4_fwd_outer1_0001.nc ]; then
      mv damee4_fwd_outer1_0001.nc ${PREFIXNOS}.fwd_outer1.$PDY.t${cyc}z.nc
      cp -p ${PREFIXNOS}.fwd_outer1.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f damee4_fwd_outer2_0001.nc ]; then
      mv damee4_fwd_outer2_0001.nc ${PREFIXNOS}.fwd_outer2.$PDY.t${cyc}z.nc
      cp -p ${PREFIXNOS}.fwd_outer2.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
    if [ -f ocean_mod.nc ]; then
      mv ocean_mod.nc ${PREFIXNOS}.mod.$PDY.t${cyc}z.nc
      cp -p ${PREFIXNOS}.mod.$PDY.t${cyc}z.nc ${COMOUT}/
    fi
#   copy back the new improved initial conditions
    initfile=${PREFIXNOS}.init.nowcast.$PDY.t${cyc}z.nc
...
#save 2D surface nowcast field output file into COMOUT
  if [ -s ${DATA}/${PREFIXNOS}.2ds.n001.$PDY.t${cyc}z.nc ]; then
...
#save nowcast station output file into COMOUT
  ...
  if [ -s $DATA/${PREFIXNOS}.avg.nowcast.$PDY.t${cyc}z.nc ]; then
    cp -p $DATA/${PREFIXNOS}.avg.n*.nc $COMOUT/
  fi 
fi
...
#    done
    if [ -f ${PREFIXNOS}.avg.nc ]; then
      mv ${PREFIXNOS}.avg.nc ${PREFIXNOS}.avg.forecast.$PDY.t${cyc}z.nc
    fi
#######################
...
#save 2D surface forecast field output file into COMOUT
  if [ -s ${DATA}/${PREFIXNOS}.2ds.f003.$PDY.t${cyc}z.nc ]; then
...
#save forecast station output file into COMOUT
  ...
  if [ -s $DATA/${PREFIXNOS}.avg.forecast.$PDY.t${cyc}z.nc ]; then
